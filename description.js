...
function loadPackageInfo() {
	...
	if (navigator.userAgent.search(/Cydia/) == -1) {
		$("#showAddRepo_").show();
		$("#open_cydia").attr("href", "cydia://url/https://cydia.saurik.com/api/share#?source=https://kurdpaxsh.ml/&package=" + urlSelfParts[1]);
	}
	$.ajax({
		url: form_url,
		type: "GET",
...
function openSection(sectionName) {
	var sectionContent = "";
	sectionContent += "<li class=\"has-icon\"><a onclick=\"loadMainSection()\" role=\"button\"><i class=\"icon f7-icons\">chevron_left</i><label>Back</label></a></li>";
	for (var keyNow in packagesSection[sectionName]) {
		...
		if (navigator.userAgent.search(/Cydia/) == -1) {
			var current_url_path = window.location.href.slice(0, window.location.href.lastIndexOf('/'));
			urlOpen = current_url_path + "/description.html?p=" + dicNow.package;
		}
...
function loadPackageDetail() {
	var packageID = window.location.href.split('description.html?p=')[1];
...
function loadRecentUpdates() {
	...
	for (var dicNow in allPackages) {
		...
		if (navigator.userAgent.search(/Cydia/) == -1) {
			var current_url_path = window.location.href.slice(0, window.location.href.lastIndexOf('/'));
			urlOpen = current_url_path + "/description.html?p=" + allPackages[dicNow].package;
		}
...
// File Size Format
! function (e, i) {
	"object" == typeof exports && "undefined" != typeof module ? module.exports = i() : "function" == typeof define && define.amd ? define(i) : (e = e || self).filesize = i()
}(this, (function () {
	"use strict";
	const e = /^(b|B)$/,
		i = {
	...
	function t(t, o = {}) {
		...
		let n = [],
			...
			N = j < 0,
			g = d > 2 ? 1e3 : 1024;
		return N && (j = -j), (-1 === v || isNaN(v)) && (v = Math.floor(Math.log(j) / Math.log(g))) < 0 && (v = 0), v > 8 && (v = 8), "exponent" === l ? v : (0 === j ? (n[0] = 0, n[1] = r ? "" : i[h][B ? "bits" : "bytes"][v]) : (a = j / (2 === d ? Math.pow(2, 10 * v) : Math.pow(1e3, v)), B && (a *= 8) >= g && v < 8 && (a /= g, v++), n[0] = Number(a.toFixed(v > 0 ? s : 0)), n[0] === g && v < 8 && void 0 === o.exponent && (n[0] = 1, v++), n[1] = 10 === d && 1 === v ? B ? "kb" : "kB" : i[h][B ? "bits" : "bytes"][v], r && (n[1] = "jedec" === h ? n[1].charAt(0) : v > 0 ? n[1].replace(/B$/, "") : n[1], e.test(n[1]) && (n[0] = Math.floor(n[0]), n[1] = ""))), N && (n[0] = -n[0]), n[1] = M[n[1]] || n[1], !0 === c ? n[0] = n[0].toLocaleString() : c.length > 0 ? n[0] = n[0].toLocaleString(c, f) : u.length > 0 && (n[0] = n[0].toString().replace(".", u)), "array" === l ? n : (m && (n[1] = p[v] ? p[v] : b[h][v] + (B ? "bit" : "byte") + (1 === n[0] ? "" : "s")), "object" === l ? {
			value: n[0],
			symbol: n[1]
		} : n.join(y)))
	}
	return t.partial = e => i => t(i, e), t
}));
...
! function (e, t) {
	...
	function f(e, t) {
		for (var n = e < 0 ? 1 : 0, r = e = Math.abs(e), a = 0; e >= c[a] && a < c.length; a++) e /= c[a];
	...
	function h(e, t, n, r) {
		...
		var u = setTimeout(function () {
			...
			for (var t = 1, n = 0, r = Math.abs(e); e >= c[n] && n < c.length; n++) e /= c[n], t *= c[n];
